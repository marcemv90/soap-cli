...
`soap-cli` is designed to be script/CI friendly. The exit code reflects the HTTP status code or
...
if [ "$status" -eq 0 ]; then
  ...
  echo "Non-HTTP error while calling soap-cli (exit code $status)" >&2
  exit 1
fi
```
...
You may want to capture and inspect the `soap-cli` exit code explicitly (as in the example above)
